...
代码是在**没有游戏、没有反编译程序集**的机器上写的，游戏 API 全部来自间接来源
（2023 年的社区数据集 + 参考 mod 源码），**没有一行在真实游戏里跑过**。

动手改代码之前，先读 [docs/04-待验证清单.md](../docs/04-待验证清单.md)：
所有猜测点按风险排序列在里面，还写了每条猜错时的表现和核对方法。

顺序是：先核对阻断级条目 → 编译 → M1 探针验证 → 再查行为级条目。
不要一上来就追行为问题。
